    ...
    while (fronts[cnt]):
        ...
        for p in fronts[cnt]:
            for q in S[p]:
                ...
                if n[q] == 0:
                    rank[q] = i+1
                    if q not in next_front: next_front.append(q)
...
def run_NSGA2(model, image, pop_size, n_generation, fitness_fn):
    ...
    offspring = do_selection_crossover_mutation(parent, rank, crowding_distance(pareto_front))  # TODO
    ...
    while (iteration < n_generation):
        ...
        offspring = do_selection_crossover_mutation(parent, rank, crowding_distance_list)
